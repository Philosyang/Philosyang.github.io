...
1. I play with zero in-game feedback sfx (i.e. the sound effects you hear from the game software when you hit a note, as well as guide sfx) and rely on the physical sound coming from the buttons when I hit them.
1. The article will work for either way of play (i.e. sound from the physical buttons vs. tap (**not** guide) sfx from the software) as long as you are determined on which one you anchor to for your playstyle.
1. The article can only be partially useful if you have the GUIDE sfx enabled and you rely on it for timings. Specifically, you should not adjust offset A under this playstyle unless you are okay with the consequences (judgement timing is de-synced with the guide sfx).
...
1. Play the song with yourself relying on the rhythm of the music as much as possible, on the chart (visual) as little as possible. Do not adjust your timings because of what you see (even if you think the chart is going too early/late;) keep your timings in sync with the rhythm (i.e. tap to the beat). You aim to tap at the same point-in-time with your eyes closed.
1. On the result screen, to the right of the top screen will show the number of fasts and lates. If there are significantly more fasts than lates (e.g., fast counts > 2 * late counts), adjust B to the negative side in steps such as from 0.0 to -0.3. If more lates than fasts, adjust B to the positive side in a similar fashion.
...
(Once again, if you rely on guide sfx, you probably would not want to adjust offset A.)
...
## extra reads
1. there is no right or wrong offset, each person defines the time to hit to the beat differently.
